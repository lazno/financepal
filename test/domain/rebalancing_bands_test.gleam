...
pub fn normal_policy_bands_test() {
  ...
  let assert Ok(equity_alloc) = allocation(0.60)
  let assert Ok(bond_alloc) = allocation(0.30)
  let assert Ok(cash_alloc) = allocation(0.10)
  
  let targets = dict.from_list([
  ...
  let assert Ok(sensitivity) = policy_sensitivity(0.20, 0.02, 0.08)
  let assert Ok(min_trade) = min_trade_value(100.0)
  let assert Ok(turnover) = turnover_cap(0.10)
  ...
  should.be_true(float.absolute_value(band_lower_bound(equity_band) -. 0.52) <. 0.001)
  should.be_true(float.absolute_value(band_upper_bound(equity_band) -. 0.68) <. 0.001)
  
  // Verify: Bond band should be [0.30 - 0.06, 0.30 + 0.06] = [0.24, 0.36]
  should.be_true(float.absolute_value(band_lower_bound(bond_band) -. 0.24) <. 0.001)
  should.be_true(float.absolute_value(band_upper_bound(bond_band) -. 0.36) <. 0.001)
  
  // Verify: Cash band should be [0.10 - 0.02, 0.10 + 0.02] = [0.08, 0.12]
  should.be_true(float.absolute_value(band_lower_bound(cash_band) -. 0.08) <. 0.001)
  should.be_true(float.absolute_value(band_upper_bound(cash_band) -. 0.12) <. 0.001)
}
...
pub fn floor_constraint_bands_test() {
  ...
  should.be_true(float.absolute_value(band_lower_bound(alt_band) -. 0.03) <. 0.001)
  should.be_true(float.absolute_value(band_upper_bound(alt_band) -. 0.07) <. 0.001)
}
...
pub fn cap_constraint_bands_test() {
  ...
  let assert Ok(equity_alloc) = allocation(0.80)
  let assert Ok(bond_alloc) = allocation(0.20)
  ...
  should.be_true(float.absolute_value(band_lower_bound(equity_band) -. 0.72) <. 0.001)
  should.be_true(float.absolute_value(band_upper_bound(equity_band) -. 0.88) <. 0.001)
}