import domain/common_types.{
  type Asset, type Symbol, Asset, asset_name, asset_name_value, isin, isin_value,
  symbol as symbol_fn, symbol_ticker,
}
...
fn insert_asset_loop(
  ...
  case assets {
    ...
    [asset, ..rest] -> {
      let sql =
        "INSERT INTO asset_registry (symbol, isin, asset_name) 
         VALUES (?, ?, ?)
         ON CONFLICT(isin) 
         DO UPDATE SET 
...
fn asset_decoder() -> decode.Decoder(Asset) {
  ...
  decode.success(Asset(symbol: symbol, isin: isin, name: asset_name))
}
