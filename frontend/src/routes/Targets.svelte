<script lang="ts">
  ...
  function handleSliderChange(index: number, newPct: number) {
    ...
    if (unpinnedRows.length > 0) {
      const totalUnpinnedBps = unpinnedRows.reduce(
        (sum, r) => sum + r.targetBps,
        0,
      );
      ...
      sortedUnpinnedIndices.forEach((realIndex, i) => {
        ...
        if (i === sortedUnpinnedIndices.length - 1) {
          ...
          const proportion =
            totalUnpinnedBps > 0
              ? row.targetBps / totalUnpinnedBps
              : 1 / unpinnedRows.length;
  ...
  function handleInputChange(index: number, value: string) {
    ...
    handleSliderChange(index, clamped);
  }

  function handleInputBlur(index: number, target: HTMLInputElement) {
    if (target.value === "" || isNaN(parseFloat(target.value))) {
      target.value = Math.round(bpsToPct(rows[index].targetBps)).toString();
    }
  }
  ...
  async function save() {
    ...
    if (Math.abs(totalTargetBps - 10000) < 5 && totalTargetBps !== 10000) {
      ...
      if (maxIndex !== -1) {
        ...
        const sensitivity = policy ? policy.sensitivity : DEFAULT_SENSITIVITY;
        rows[maxIndex].band = calculateBand(
          rows[maxIndex].targetBps,
          sensitivity,
        );
      }
    }
...
<div class="container mx-auto max-w-5xl pb-20">
  ...
  <div class="flex justify-between items-start mb-10">
    <div class="header-content">
      <h1
        class="text-4xl font-bold mb-2 bg-clip-text text-transparent bg-gradient-to-br from-primary-500 to-primary-300 tracking-tighter"
      >
        Target Allocation
      </h1>
    ...
    <div
      class="band-badge px-4 py-2 rounded-lg text-sm font-semibold backdrop-blur-md"
    >
      {#if policy}
  ...
  {:else}
    ...
    <div class="projected-draft mb-10 rounded-2xl p-8 backdrop-blur-md">
      <div class="section-title text-xl font-bold mb-6 flex items-center gap-3">
        Allocation Analysis
      </div>
      <div class="flex gap-6 mb-6 flex-wrap">
        <div
          class="flex items-center gap-2 text-sm font-medium text-surface-600 dark:text-surface-400"
        >
    ...
    <div
      class="hidden md:block table-wrapper mb-8 rounded-2xl overflow-hidden backdrop-blur-md"
    >
      <table class="w-full border-collapse">
        <thead>
          <tr>
            <th
              class="px-4 py-5 text-left text-xs font-bold uppercase tracking-wider"
              >Instrument Type</th
            >
            <th
              class="px-4 py-5 text-left text-xs font-bold uppercase tracking-wider"
            ...
            >
            <th
              class="px-4 py-5 text-left text-xs font-bold uppercase tracking-wider w-1/2"
              >Target Allocation</th
            >
            <th
              class="px-4 py-5 text-center text-xs font-bold uppercase tracking-wider w-32"
            ></th>
          </tr>
        </thead>
        <tbody>
          {#each rows as row, i}
            <tr
              class="hover:bg-primary-500/5 dark:hover:bg-primary-500/10 transition-colors"
            >
              <td
                class="px-4 py-5 text-sm border-b border-primary-500/10 dark:border-primary-300/10 instrument-name font-bold tracking-wide"
                >{row.instrumentType}</td
              >
              <td
                class="px-4 py-5 text-sm border-b border-primary-500/10 dark:border-primary-300/10 current-pct font-bold text-base"
              >
                {row.currentBps === 0
                  ? "—"
              ...
              <td
                class="px-4 py-5 text-sm border-b border-primary-500/10 dark:border-primary-300/10"
              >
                <div class="slider-container flex items-center gap-3 w-full">
                  <input
                    ...
                    oninput={(e) =>
                      handleSliderChange(i, parseFloat(e.currentTarget.value))}
                    disabled={row.pinned}
                  />
                  <div class="flex items-center min-w-[3.5rem] justify-end">
                    <input
                      type="number"
                      class="w-12 bg-transparent text-right font-bold text-base focus:outline-none focus:ring-1 focus:ring-primary-500/50 rounded px-0.5 target-input"
                      min="0"
                      max="100"
                      value={Math.round(bpsToPct(row.targetBps))}
                      oninput={(e) =>
                        handleInputChange(i, e.currentTarget.value)}
                      onblur={(e) => handleInputBlur(i, e.currentTarget)}
                      disabled={row.pinned}
                    />
                    <span class="text-sm font-bold ml-0.5 text-surface-500"
                      >%</span
                    >
                  </div>
                </div>
              ...
              <td
                class="px-4 py-5 text-sm border-b border-primary-500/10 dark:border-primary-300/10 text-center"
              >
                <div class="flex items-center justify-end gap-2 pr-4">
                  <button
                    class="pin-btn w-8 h-8 rounded-md flex items-center justify-center transition-colors duration-200 {row.pinned
                      ? 'pinned'
                      : ''}"
                    onclick={() => togglePin(i)}
          ...
          <tr class="total-row font-bold">
            ...
            <td class="border-none py-6 text-right">
              <div
                class="flex items-center justify-end gap-2 text-lg font-bold {isValid
                  ? 'text-success-500'
                  : 'text-error-500'}"
              >
                <span
                  class="total-indicator w-2 h-2 rounded-full inline-block mr-2 {isValid
                    ? 'bg-success-500'
    ...
    <div class="md:hidden space-y-4 mb-8">
      {#each rows as row, i}
        <div
          class="glass-card p-4 rounded-xl border border-primary-500/10 relative overflow-hidden"
        >
          <!-- Row 1: Name + Current + Pin -->
          <div class="flex justify-between items-start mb-4">
            <div>
              <div class="font-bold text-lg tracking-wide mb-1">
                {row.instrumentType}
              </div>
              <div
              ...
              >
                Current: <span
                  ...
                  >{row.currentBps === 0
                    ? "—"
                    : bpsToPct(row.currentBps).toFixed(2) + "%"}</span
                >
              </div>
            </div>
            <div class="flex gap-2">
              <button
                class="w-8 h-8 rounded-lg bg-surface-100 dark:bg-surface-800 flex items-center justify-center transition-colors {row.pinned
          ...
          <div class="mb-2">
            <div
              class="flex justify-between text-xs font-medium mb-2 text-surface-500 items-end"
            >
              <span>Target Allocation</span>
              <div class="flex items-center">
                <input
                  type="number"
                  class="w-12 bg-transparent text-right text-lg font-bold text-primary-600 dark:text-primary-400 focus:outline-none focus:border-b focus:border-primary-500 p-0 rounded-none"
                  min="0"
                  max="100"
                  value={Math.round(bpsToPct(row.targetBps))}
                  oninput={(e) => handleInputChange(i, e.currentTarget.value)}
                  disabled={row.pinned}
                />
                <span
                  class="text-sm font-bold ml-0.5 text-primary-600 dark:text-primary-400"
                  >%</span
                >
              </div>
            </div>
      ...
      <!-- Mobile Total -->
      <div
        class="glass-card p-4 rounded-xl border border-primary-500/10 flex justify-between items-center"
      >
        <span class="font-bold text-surface-600 dark:text-surface-300"
          >Total Allocation</span
        >
        <div
          class="flex items-center gap-2 text-lg font-bold {isValid
            ? 'text-success-500'
            : 'text-error-500'}"
        >
          <span
            class="w-2 h-2 rounded-full {isValid
              ? 'bg-success-500'
              : 'bg-error-500 animate-pulse'}"
          ></span>
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-5">
      <!-- Add Instrument -->
      <div
        class="relative h-12 flex items-center w-full md:w-auto justify-center md:justify-start order-2 md:order-1"
      >
        {#if isAddingInstrument}
          <div
            class="glass-input-container flex items-center gap-2 p-1.5 rounded-lg animate-fade-in w-full md:w-auto justify-between"
          >
            <input
              type="text"
            ...
            <div class="flex gap-1">
              <button
                class="w-8 h-8 rounded-md bg-primary-500 text-white flex items-center justify-center hover:bg-primary-600 transition-colors shadow-sm"
                onclick={addInstrument}
              >
                <IconPlus class="w-5 h-5" />
              </button>
              <button
                class="w-8 h-8 rounded-md flex items-center justify-center text-surface-500 hover:text-error-500 hover:bg-error-500/10 transition-colors"
                onclick={() => (isAddingInstrument = false)}
              >
                <IconX class="w-4 h-4" />
              </button>
            </div>
        ...
        {:else}
          ...
          {#if showAddTooltip}
            <div
              class="tooltip absolute bottom-full left-1/2 md:left-0 transform -translate-x-1/2 md:translate-x-0 mb-2 px-3 py-2 rounded bg-surface-900 text-white text-xs whitespace-nowrap z-50 shadow-lg pointer-events-none"
            >
              Add Instrument
            </div>
          {/if}
      ...
      <div
        class="grid grid-cols-1 sm:grid-cols-3 gap-3 w-full md:w-auto order-1 md:order-2"
      >
        <button
          class="btn px-6 py-3 rounded-lg font-bold text-sm transition-all duration-300 shadow-sm btn-secondary flex items-center justify-center"
          onclick={resetToCurrent}
        >
          <IconRefreshCw class="mr-2 w-4 h-4" />
        ...
        <button
          ...
          title={!policy || policy.id === "new"
            ? "No saved policy to delete"
...
{#if showDeleteModal}
  <div
    class="modal fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm active"
  >
    <div class="modal-content w-full max-w-md p-8 rounded-2xl shadow-2xl">
      ...
      <div class="modal-text mb-6 text-sm opacity-80">
        Are you sure you want to delete this target allocation? This action
        cannot be undone.
      </div>
      <div class="modal-actions flex gap-3">
        <button
          class="btn px-6 py-3 rounded-lg font-bold text-sm transition-all duration-300 shadow-sm btn-secondary flex-1"
          onclick={() => (showDeleteModal = false)}>Cancel</button
        >
        <button
          class="btn px-6 py-3 rounded-lg font-bold text-sm transition-all duration-300 shadow-sm btn-danger flex-1"
          onclick={removePolicy}>Delete</button
        >
      </div>
    </div>
  </div>
...
<style lang="postcss">
  ...
  :global([data-mode="dark"]) thead {
    background: linear-gradient(
      90deg,
      rgba(212, 165, 255, 0.1) 0%,
      rgba(183, 139, 255, 0.05) 100%
    );
  ...
  :global([data-mode="dark"]) .target-input {
    color: var(--color-primary-300);
  }
  ...
  :global([data-mode="dark"]) .total-row {
    background: linear-gradient(
      90deg,
      rgba(212, 165, 255, 0.15) 0%,
      rgba(183, 139, 255, 0.08) 100%
    );
